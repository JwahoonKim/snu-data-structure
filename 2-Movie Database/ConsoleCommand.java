...
abstract class AbstractConsoleCommand implements ConsoleCommand {
	...
	/**
	 * {@link AbstractConsoleCommand.parse} 메소드에서 분해된 문자열 배열(String[]) 을 이용해 인자를
	 * 해석하는 추상 메소드.
	 * 
	 * 자식 클래스들은 parse 메소드가 아니라 이 메소드를 오버라이드하여 각 명령에 맞는 규칙으로 인자를 해석한다.
	 * 
...
/******************************************************************************
 * 아래의 코드는 ConsoleCommand 에서 사용하는 익셉션들의 모음이다. 필요하면 수정해도 좋으나 수정하지 않아도 된다.
 *****************************************************************************/

/******************************************************************************
 * ConsoleCommand 처리 중에 발생하는 익셉션의 상위 클래스이다. {@code throws} 구문이나 {@code catch}
 * 구문을 간단히 하는데 사용된다.
 */
